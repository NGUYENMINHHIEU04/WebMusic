...
function ToggleBox({ isActive, toggleForm }) {
  return (
    <div className="absolute w-full h-full">
      <div className={`absolute w-[300%] h-full bg-green-900 rounded-full transition-all duration-1000 ease-in-out ${isActive ? 'left-1/2' : '-left-[250%]'}`}></div>
      <div className={`absolute w-1/2 h-full flex flex-col justify-center items-center text-white transition-all duration-1000 ease-in-out ${isActive ? '-left-1/2' : 'left-0'}`}>
        ...
        <button onClick={toggleForm} className="w-40 py-2 bg-transparent border-2 border-white rounded-lg text-white font-semibold text-lg hover:bg-white hover:text-green-500 transition-all">
          Register
        </button>
      </div>
