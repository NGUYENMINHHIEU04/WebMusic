...
// import { getArtistsByIds } from '../apis/api_artist'; // Thêm import
// import { FaPlay, FaPause } from 'react-icons/fa';
// import { FiClock } from 'react-icons/fi';

// const PlaylistDetail = ({ playlist, onBack, isPlaying, setIsPlaying, onTrackSelect, onArtistSelect }) => {
...
//                 artistIds: songData.artistIds || [], // Lưu artistIds để sử dụng
//               };
//             } catch (err) {
...
//       // Khi chọn track, lấy thông tin nghệ sĩ
//       if (track.artistIds && track.artistIds.length > 0) {
//         const artists = await getArtistsByIds(track.artistIds);
//         onArtistSelect(artists[0]); // Chọn nghệ sĩ đầu tiên trong danh sách
//       }
  ...
  const handleTrackPlayPause = async (track) => {
    try {
      ...
      if (currentTrackId === track.id) {
        // Nếu bài hát hiện tại được bấm, chỉ toggle play/pause
        setIsPlaying(!isPlaying);
      } else {
        // Nếu chọn bài hát mới, cập nhật bài hát và reset thời gian
        setCurrentTrackId(track.id);
        setIsPlaying(true);
        resetCurrentTime();
        onTrackSelect(track, tracks);
