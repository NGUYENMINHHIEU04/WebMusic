...
//   const audioRef = useRef(new Audio(song.url)); // Khởi tạo với URL bài hát mặc định
//   const progressRef = useRef(null);

//   // Cập nhật audio src khi song.url thay đổi
//   useEffect(() => {
...
//     if (volume === 0)
//       return (
//         <FiVolumeX className="w-5 h-5 text-gray-400 hover:text-white hover:scale-110 transition-transform duration-200" />
//       );
//     else if (volume <= 0.3)
//       return (
//         <FiVolume className="w-5 h-5 text-gray-400 hover:text-white hover:scale-110 transition-transform duration-200" />
//       );
//     else if (volume <= 0.6)
//       return (
//         <FiVolume1 className="w-5 h-5 text-gray-400 hover:text-white hover:scale-110 transition-transform duration-200" />
//       );
//     else
//       return (
//         <FiVolume2 className="w-5 h-5 text-gray-400 hover:text-white hover:scale-110 transition-transform duration-200" />
//       );
//   };
...
//               />
//               <div>
//                 <p className="font-semibold">{song.title}</p> {/* Hiển thị tiêu đề bài hát */}
//                 <p className="text-sm text-gray-400">{song.artist}</p> {/* Hiển thị nghệ sĩ */}
//               </div>
...
//                   <div className="relative group">
//                     <FaCirclePause
//                       className="w-8 h-8 text-white cursor-pointer hover:scale-110 transition-transform duration-200"
//                       onClick={togglePlay}
//                     />
//                     <span className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 opacity-0 group-hover:opacity-100 transition-opacity bg-gray-800 text-white text-xs font-bold rounded-lg px-3 py-1 shadow-lg whitespace-nowrap">
//                       Pause
...
//                   {repeatMode === 'inactive' ? (
//                     <LuRepeat
//                       className="w-5 h-5 cursor-pointer text-gray-400 hover:text-white hover:scale-110 transition-transform duration-200"
//                       onClick={toggleRepeat}
//                     />
//                   ) : repeatMode === 'repeat' ? (
//                     <LuRepeat
//                       className="w-5 h-5 cursor-pointer text-green-500 hover:scale-110 transition-transform duration-200"
//                       onClick={toggleRepeat}
//                     />
//                   ) : (
//                     <LuRepeat1
...
//                 <AiOutlinePlaySquare
//                   className={`w-5 h-5 cursor-pointer ${
//                     activeRightIcon === 'singer'
//                       ? 'text-green-500'
//                       : 'text-gray-400 hover:text-white'
//                   } hover:scale-110 transition-transform duration-200`}
//                   onClick={() => {
//                     toggleRightIcon('singer');
//                     onToggleSingerInfo();
//                   }}
//                 />
//                 {activeRightIcon === 'singer' && (
...
//                 <HiMiniQueueList
//                   className={`w-5 h-5 cursor-pointer ${
//                     activeRightIcon === 'queue'
//                       ? 'text-green-500'
//                       : 'text-gray-400 hover:text-white'
//                   } hover:scale-110 transition-transform duration-200`}
//                   onClick={() => {
//                     toggleRightIcon('queue');
//                     onToggleQueue();
//                   }}
//                 />
//                 {activeRightIcon === 'queue' && (
...
//                 <MdImportantDevices
//                   className={`w-5 h-5 cursor-pointer ${
//                     activeRightIcon === 'device'
//                       ? 'text-green-500'
//                       : 'text-gray-400 hover:text-white'
//                   } hover:scale-110 transition-transform duration-200`}
//                   onClick={() => {
//                     toggleRightIcon('device');
//                     onToggleDevice();
//                   }}
//                 />
//                 {activeRightIcon === 'device' && (
...
const MusicPlayer = ({
  ...
  const toggleRepeat = () => {
    setRepeatMode((prevMode) =>
      prevMode === 'inactive' ? 'repeat' : prevMode === 'repeat' ? 'repeat1' : 'inactive'
    );
  };

  const toggleRightIcon = (icon) => {
    setActiveRightIcon(activeRightIcon === icon ? null : icon);
  };
    ...
    >
      {isFullScreen ? (
        <MusicPlayerFullScreen
          ...
          onToggleDevice={onToggleDevice}
          toggleFullScreen={toggleFullScreen}
          songTitle={song?.title || 'No Song Selected'}
          lyrics={song?.lyrics || ''}
        />
      ) : (
        <div className="container mx-auto flex items-center justify-between">
          ...
          <div className="flex flex-col items-center space-y-2 w-[40%]">
            <div className="flex items-center space-x-4">
              <div className="relative group">
                <RxShuffle
                  className={`w-5 h-5 cursor-pointer ${
                    isShuffleActive ? 'text-green-500' : 'text-gray-400 hover:text-white'
                  } hover:scale-110 transition-transform duration-200`}
                  onClick={toggleShuffle}
                />
                {isShuffleActive && (
            ...
            <div className="flex items-center space-x-2 w-[90%]">
              <span className="text-sm text-gray-400">{song ? formatTime(currentTime) : '0:00'}</span>
              <div
                ref={progressRef}
                onClick={handleSeek}
                className="h-1 w-full bg-gray-600 rounded-full cursor-pointer"
              ...
              <span className="text-sm text-gray-400">{song ? formatTime(duration) : '0:00'}</span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-end space-x-4 w-[30%]">
            <div className="relative group">
              <AiOutlinePlaySquare
                className={`w-5 h-5 cursor-pointer ${
                  activeRightIcon === 'singer' ? 'text-green-500' : 'text-gray-400 hover:text-white'
                } hover:scale-110 transition-transform duration-200`}
                onClick={() => {
                  toggleRightIcon('singer');
                  onToggleSingerInfo();
            ...
            <div className="relative group">
              <MdLyrics
                className={`w-6 h-5 cursor-pointer ${
                  isLyricsVisible ? 'text-green-500' : 'text-gray-400 hover:text-white'
                } hover:scale-110 transition-transform duration-200`}
                onClick={handleToggleLyrics}
              />
              {isLyricsVisible && (
            ...
            <div className="relative group">
              <HiMiniQueueList
                className={`w-5 h-5 cursor-pointer ${
                  activeRightIcon === 'queue' ? 'text-green-500' : 'text-gray-400 hover:text-white'
                } hover:scale-110 transition-transform duration-200`}
                onClick={() => {
                  toggleRightIcon('queue');
                  onToggleQueue();
            ...
            <div className="relative group">
              <MdImportantDevices
                className={`w-5 h-5 cursor-pointer ${
                  activeRightIcon === 'device' ? 'text-green-500' : 'text-gray-400 hover:text-white'
                } hover:scale-110 transition-transform duration-200`}
                onClick={() => {
                  toggleRightIcon('device');
                  onToggleDevice();
      ...
      {showMiniPlayer && (
        <MiniPlayer isPlaying={isPlaying} togglePlay={togglePlay} onClose={toggleMiniPlayer} />
      )}
    </div>
  );
};
